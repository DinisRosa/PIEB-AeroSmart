  ...
  if (desktop) {
    return (
      <nav className="flex flex-col gap-1 px-3 py-4 flex-1">
        <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest px-3 mb-2">Navegação</p>
        {tabs.map(tab => {
          ...
          return (
            <Link
              ...
              className={`
                ...
                ${isActive
                  ? 'bg-soft-blue text-brand-blue shadow-sm'
                  : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
                }
              `}
            >
              <span className={`flex-shrink-0 ${isActive ? 'text-brand-blue' : 'text-slate-400'}`}>
          ...
          >
            {isActive ? (
              <div className="w-6 h-6 flex items-center justify-center bg-soft-blue rounded-lg">
                <span style={{ transform: 'scale(0.75)', display: 'block' }}>
                  {tab.activeIcon}
                </span>
              </div>
            ) : (
